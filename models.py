...
class IMLE(nn.Module):
    def __init__(self, H):
        ...
        if use_unet:
            ...
            # 1. If condition_path is provided (normal mode)
            # 2. If using teacher mode (teacher generates conditions)
            # 3. NOT if using teacher noise as input (unconditional mode)
            use_teacher_noise_as_input = hasattr(H, 'use_teacher_noise_as_input') and H.use_teacher_noise_as_input
            use_teacher_mode = (hasattr(H, 'use_teacher_resample') and H.use_teacher_resample and
                               hasattr(H, 'teacher_generate_initial_data') and H.teacher_generate_initial_data)
            use_conditioning = (H.condition_path is not None) or (use_teacher_mode and not use_teacher_noise_as_input)
            ...
            if use_teacher_noise_as_input:
                ...
                print(f"✓ Using UNCONDITIONAL UNet - PURE REGRESSION MODE")
                print(f"  Mode: Direct mapping teacher_noise → teacher_output")
                print(f"  Latent codes are ignored. Use with force_factor=1 for regression.")
                print(f"  Architecture: NO conditioning layers, purely unconditional")
            elif use_teacher_mode and H.condition_path is None:
                print(f"Using UNet model (conditional: {use_conditioning}) - Teacher mode: conditions generated by teacher")
            else:
    ...
    def forward(self, latents, spatial_noise=None, input_is_w=False, condition_data=None, condition_indices=None, return_condition=False, return_latent_only=False):
        ...
        if is_unet:
            # Check if we should use teacher noise as input
            use_teacher_noise_as_input = hasattr(self.H, 'use_teacher_noise_as_input') and self.H.use_teacher_noise_as_input
            
            if use_teacher_noise_as_input and condition_data is not None:
                # PURE REGRESSION MODE with UNCONDITIONAL UNet
                # Input: teacher's x0 noise
                # Output: teacher's output
                # Latents: IGNORED (not used)
                # Conditioning: NONE (unconditional architecture)
                
                # Use teacher's x0 noise as the direct input
                if condition_data.ndim != 4:
                    batch_size = condition_data.shape[0]
                    latent_side = self.img_size // 8
                    input_tensor = condition_data.reshape(batch_size, self.latent_channels, latent_side, latent_side)
                else:
                    input_tensor = condition_data
                
                # UNCONDITIONAL: No conditioning embedding
                condition_emb = None
            else:
                # Normal mode: use latents as input
                if latents.ndim != 4:
                    batch_size = latents.shape[0]
                    latent_side = self.img_size // 8
                    input_tensor = latents.reshape(batch_size, self.latent_channels, latent_side, latent_side)
                else:
            ...
            out = self.decoder(input_tensor, condition_emb, return_latent_only=return_latent_only)
